...
##Appendix: how would the XGBoost Model look if we only take the first 5 variables shown in the variable importance graph
 
 We have decided to run another XGBoost model only with the top 5 variables that added more to the model just to compare it with the initial in case it would be overfitting the data.
 The variables used were: "mt, category_grocery_pos, hour, category_gas_transport, age". 
 
 However, while plotting the Mean_ROC_AUC, we noticed that it was exactly the same as the initial model, and ergo it wouldn't make a lot of difference to have this model with only 5 variables compared with the original one and finally decided to keep the original one. 
 
 See code below with this new model.

```{r}

##Creating a model using only the top 5 variables:  amt, category_grocery_pos, hour, category_gas_transport, age

#Step1: new recipe only with the 5 variables
fraud_rec_final <- recipe(is_fraud ~ ., data = card_fraud_train) %>% 
  step_log(amt) %>% 
  step_novel(all_nominal(), -all_outcomes()) %>% # Use *before* `step_dummy()` so new level is dummified
  step_dummy(all_nominal(), -all_outcomes()) %>% 
  step_zv(all_numeric(), -all_outcomes())  %>% 
  step_normalize(all_numeric()) %>% 
  step_select("is_fraud","amt","category_grocery_pos","hour","category_gas_transport","age")

#Step 2: creating the xgb_wflow,  xgb_res and xgb_metrics

xgb_wflow_final <-
 workflow() %>%
 add_recipe(fraud_rec_final) %>% 
 add_model(xgb_spec)
...
time <- toc()
xgb_time_final <- time[[4]]


xgb_metrics_final <- 
  xgb_res_final %>% 
  collect_metrics(summarise = TRUE) %>%
  mutate(model = "XGBoost_Final",
         time = xgb_time_final)
...
```{r}
"Percentage of fraudulent transactions by type of merchant in % of total"
card_fraud %>% 
  filter(is_fraud==1) %>% 
  count(category) %>% 
...
```{r}
#Age intervals of people who were victims in a fraudulent transaction
card_fraud %>% 
  filter(is_fraud==1) %>% 
